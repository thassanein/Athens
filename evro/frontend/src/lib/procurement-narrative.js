...
function bookNarrative(db) {
  ...
  return {
    ...
    figures: [
      { label: 'Run-rate under mgmt', value: `${money(sum.total)}/yr` },
      { label: 'Validated YTD', value: money(sum.lenses.realized) },
      { label: 'At risk', value: money(sum.atRisk) },
      { label: 'Confidence', value: pct(sum.confidence) },
    ],
    sections: [
      { heading: 'Position', paragraphs: [
        `Procurement is managing ${money(sum.total)} of annualized run-rate across ${num(sum.count)} opportunities at ${pct(sum.confidence)} value-weighted confidence — the book's worth per year at full delivery, each saving measured over a 12-month window from first financial reporting. Of that, ${money(sum.lenses.identified)} is identified and ${money(sum.lenses.committed)} committed into the plan, while ${money(sum.lenses.realized)} has been FP&A-validated year-to-date — the only portion banked so far, landing at ${money(vel.perMonth)} a month over ${num(vel.elapsedMonths)} elapsed months.`,
